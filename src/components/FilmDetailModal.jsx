...
export default function FilmDetailModal({ omdbData, onClose, onLike, onNope }) {
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {poster && (
              <div className="relative h-48 sm:h-64">
                <img src={poster} alt={omdbData.Title} className="w-full h-full object-cover object-top" />
            ...
            <div className="px-5 pb-8 space-y-4">
              ...
              <div className="flex items-start justify-between gap-3 pt-2">
                <h2 className="text-white text-2xl font-black leading-tight flex-1">
                  {omdbData.Title}
                </h2>
                <button
                  onClick={onClose}
                  className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-gray-400 hover:text-white shrink-0 mt-0.5"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              {val(omdbData.Genre) && (
                <div className="flex flex-wrap gap-1.5">
                  {omdbData.Genre.split(', ').map((g) => (
                    <span key={g} className="text-xs px-2.5 py-1 rounded-full bg-white/8 text-gray-300 border border-white/8">
                      {g}
                    </span>
                  ))}
                </div>
              ...
              {val(omdbData.Director) && (
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="text-gray-500 text-xs">Direção · </span>
                    <span className="text-gray-300 text-sm">{omdbData.Director}</span>
                  </div>
                </div>
              )}
              ...
              {val(omdbData.Actors) && (
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="text-gray-500 text-xs">Elenco · </span>
                    <span className="text-gray-300 text-sm">{omdbData.Actors}</span>
                  </div>
                </div>
              )}
              ...
              {(val(omdbData.Country) || val(omdbData.Language)) && (
                <div className="text-gray-500 text-xs">
                  {val(omdbData.Country) && <span>{omdbData.Country}</span>}
                  {val(omdbData.Country) && val(omdbData.Language) && <span> · </span>}
                  {val(omdbData.Language) && <span>{omdbData.Language}</span>}
                </div>
              )}
              ...
              {(onLike || onNope) && (
                <div className="flex gap-3 pt-2">
                  {onNope && (
                    <button
                      onClick={onNope}
                      className="flex-1 py-3.5 rounded-2xl border border-rose-500/40 text-rose-400 font-semibold flex items-center justify-center gap-2 hover:bg-rose-500/10 transition-colors"
                    >
                      <X size={18} />
                      Passar
                    </button>
                  ...
                  {onLike && (
                    <button
                      onClick={onLike}
                      className="flex-1 py-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-400/40 text-emerald-400 font-semibold flex items-center justify-center gap-2 hover:bg-emerald-500/20 transition-colors"
                    >
                      <Heart size={18} />
                      Curtir
                    </button>
