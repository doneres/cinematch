    ...
    <div className="flex flex-col items-center w-full">
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl bg-[#12121f] border border-white/5">
          ...
          <div className="px-4 py-3 space-y-2">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400 flex-wrap">
              ...
              {film?.rating > 0 && <span className="flex items-center gap-1 text-amber-400"><Star size={11} fill="currentColor" />{film.rating}</span>}
            </div>
            {genres.length > 0 && (
      ...
      {isTop && (
        <div className="flex gap-8 mt-4">
          <button onClick={() => handleButton('nope')} className="w-14 h-14 rounded-full bg-[#1a1a2e] border border-rose-500/40 text-rose-500 flex items-center justify-center hover:bg-rose-500/10 transition-colors shadow-lg active:scale-90">
            <X size={24} strokeWidth={2.5} />
          </button>
          <button onClick={() => handleButton('like')} className="w-14 h-14 rounded-full bg-[#1a1a2e] border border-emerald-400/40 text-emerald-400 flex items-center justify-center hover:bg-emerald-400/10 transition-colors shadow-lg active:scale-90">
